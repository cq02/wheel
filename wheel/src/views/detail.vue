<template>
  <div class="pages">
    <div class="header">
      <router-link :to="{
                path:'/img',
                query:{id:list.SerialID}}"><img :src="list.CoverPhoto" alt="" class="img"></router-link>
    </div>
    <section>
      <!--图片下面的第一个div-->
      <div class="top1">
        <div class="pri">
          <span class="pris">{{list.market_attribute.dealer_price && list.market_attribute.dealer_price}}</span>
          <span class="pictor">指导价{{list.market_attribute.official_refer_price && list.market_attribute.official_refer_price}}价</span>
        </div>
        <router-link :to="{path:'/price',
                                                    query:{id:list.BottomEntranceLink.split('/')[list.BottomEntranceLink.split('/').length-1]}}">
      ...
      <div v-for='(value,key) in all' :key='key' class="all">
        ...
        <div class="btn">询问低价</div>
      </div>
    </section>
    <router-link :to="{path:'/price',
                                     query:{id:list.BottomEntranceLink.split('/')[list.BottomEntranceLink.split('/').length-1]}}">
      <div class="fixBtn">
        {{list.BottomEntranceTitle}}
        <h6>本地经销商为你报价</h6>
      </div>
    </router-link>
  </div>
